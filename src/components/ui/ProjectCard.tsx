...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-day-border dark:border-agency-border bg-day-surface dark:bg-agency-surface transition-all duration-300 hover:border-digitify-purple/50 dark:hover:border-digitify-purple/50 flex flex-col group shadow-sm">
      ...
      <div className="flex items-center justify-between px-3.5 sm:px-4 py-2.5 border-b border-day-border/70 dark:border-agency-border/70 bg-day-surface/95 dark:bg-[#0d0d12]/95 backdrop-blur-sm z-10 select-none">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-day-border/40 dark:bg-agency-border/40 border border-day-border/60 dark:border-agency-border/60 text-[11px] font-mono text-day-subtext dark:text-agency-subtext tracking-wide truncate max-w-[170px] xs:max-w-[210px] sm:max-w-[240px]">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isPdf ? (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider bg-digitify-purple/10 text-digitify-purple border border-digitify-purple/20">
              <FileText size={10} />
              <span className="hidden sm:inline">Deck</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="hidden sm:inline">Live</span>
            </span>
          )}

          {/* Optional subtle Case Study action — shown ONLY if caseStudyUrl exists */}
          {project.caseStudyUrl && (
            <Link
              to={project.caseStudyUrl}
              className="inline-flex items-center gap-0.5 text-[11px] font-mono text-digitify-purple hover:underline transition-colors shrink-0 ml-1"
            >
          ...
          {project.externalLink && (
            <a
              ...
              className="text-day-muted dark:text-agency-muted hover:text-black dark:hover:text-white transition-colors p-0.5"
      ...
      {/* Main Visual Preview Area (Contained Fixed-Height Viewport) */}
      <div className="relative w-full h-[420px] sm:h-[480px] lg:h-[520px] overflow-hidden bg-black/95 dark:bg-[#0c0c10]">
        {isPdf ? (
          ...
          <div className="w-full h-full relative overflow-y-auto overscroll-contain bg-[#111116] scrollbar-thin">
            ...
            <div className="absolute bottom-2.5 right-3 pointer-events-none text-[10px] font-mono text-white/70 bg-black/80 backdrop-blur-sm px-2.5 py-1 rounded-md border border-white/10 select-none flex items-center gap-1.5 shadow-sm">
          ...
          <div className="w-full h-full relative overflow-hidden bg-white dark:bg-black">
            ...
            <div className="absolute bottom-0 inset-x-0 bg-black/85 backdrop-blur-md px-3 py-1.5 flex items-center justify-between text-[11px] font-mono text-zinc-300 border-t border-white/10 z-10 select-none">
              <span className="truncate pr-2 text-zinc-400">
                {project.fallbackMessage || 'Live embedded preview · Interacting stays contained inside card'}
              </span>
              {project.externalLink && (
                <a
                  href={project.externalLink}
