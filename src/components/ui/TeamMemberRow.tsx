...
export const TeamMemberRow: React.FC<TeamMemberRowProps> = ({ member }) => {
  return (
    <Link
      ...
      className="group relative block w-full py-4 sm:py-5 px-3 sm:px-5 border-b border-day-border dark:border-agency-border transition-all duration-400 ease-out hover:bg-day-surface/60 dark:hover:bg-agency-surface/50 focus:bg-day-surface/60 dark:focus:bg-agency-surface/50 focus:outline-none"
    >
      <div className="flex items-center justify-between gap-3 sm:gap-6 transition-transform duration-400 ease-out group-hover:translate-x-2 sm:group-hover:translate-x-3">
        {/* Left: Index, Profile Photo Placeholder & Name */}
        <div className="flex items-center gap-3 sm:gap-5 min-w-0">
          {/* Index Number */}
          <span className="text-xs sm:text-sm font-mono text-day-muted dark:text-agency-muted group-hover:text-digitify-purple transition-colors duration-300 w-5 shrink-0">
            {member.number}
          </span>

          {/* Profile Photo Placeholder Area */}
          <div
            className="relative w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-lg border border-day-border dark:border-agency-border bg-day-surface dark:bg-agency-surface shrink-0 flex flex-col items-center justify-center overflow-hidden transition-all duration-400 group-hover:border-black/40 dark:group-hover:border-white/40"
            title={`Editable photo placeholder for ${member.name}`}
          >
            {/* Minimalist Monogram / Placeholder Graphic */}
            <div className="flex flex-col items-center justify-center select-none text-center p-1">
              <span className="text-xs sm:text-sm font-mono font-semibold tracking-wider text-black dark:text-white group-hover:text-digitify-purple transition-colors duration-300">
                {member.initials}
              </span>
              <span className="text-[8px] font-mono uppercase tracking-tighter text-day-muted dark:text-agency-muted">
                PHOTO
              </span>
            </div>
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base sm:text-lg md:text-xl font-display font-medium text-black dark:text-white tracking-tight truncate group-hover:text-black dark:group-hover:text-white transition-colors duration-300">
                {member.name}
              </h3>
            </div>
            <span className="text-xs font-mono uppercase tracking-wider text-day-subtext dark:text-agency-subtext block sm:hidden mt-0.5">
              {member.role}
            </span>
          </div>
        </div>
