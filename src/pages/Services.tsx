...
export const Services: React.FC = () => {
  return (
    <div className="w-full max-w-7xl mx-auto px-3.5 xs:px-4 sm:px-8 md:px-12 py-6 xs:py-8 sm:py-12 flex flex-col space-y-10 xs:space-y-14 sm:space-y-18 md:space-y-20">
      {/* Header */}
      ...
      <section className="border-t border-day-border dark:border-agency-border divide-y divide-day-border dark:divide-agency-border">
        {servicesData.map((service) => (
          <div
            key={service.id}
            id={service.id}
            className="scroll-mt-16 xs:scroll-mt-20 sm:scroll-mt-28 py-8 xs:py-10 sm:py-14 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-start relative"
          >
            {service.id === 'branding' && <span id="branding-identity" className="absolute -top-16 xs:-top-20 sm:-top-28" />}
            {service.id === 'social-media' && <span id="social-media-management" className="absolute -top-16 xs:-top-20 sm:-top-28" />}
            {service.id === 'web-design' && <span id="web-design-experiences" className="absolute -top-16 xs:-top-20 sm:-top-28" />}
            ...
            <div className="lg:col-span-5 flex flex-col space-y-2.5 sm:space-y-3">
              ...
              <h2 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-display font-medium text-black dark:text-white tracking-tight leading-tight">
                {service.name}
              </h2>
              <p className="text-xs sm:text-sm text-day-subtext dark:text-agency-subtext leading-relaxed pt-0.5">
                {service.shortDesc}
              </p>
              ...
              <div className="pt-2 sm:pt-4">
                <Link
                  to="/contact"
                  className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-2.5 rounded-full border border-day-border dark:border-agency-border text-xs uppercase tracking-widest text-black dark:text-white hover:border-black dark:hover:border-white active:scale-95 transition-colors"
                >
